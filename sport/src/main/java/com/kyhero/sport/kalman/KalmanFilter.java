package com.kyhero.sport.kalman;

import android.graphics.PointF;
import android.util.Log;

import com.kyhero.sport.kalman.jama.Matrix;
import com.kyhero.sport.kalman.jkalman.JKalman;
...
public class KalmanFilter {
    ...
    // 可以过滤掉跳变值。
    public PointF filter(PointF oldValue) {

        PointF newValue = null;
        Log.d(TAG, "filter: x "+oldValue.x+" > "+oldValue.y);
        // check state before
        ...
        float x = (float) mPredictValue.get(0, 0);
        float y = (float) mPredictValue.get(1, 0);
        newValue=new PointF(x,y);
        Log.d(TAG, "filter: x "+oldValue.x+" > "+newValue.x+" y "+oldValue.y+" > "+newValue.y);
        return newValue;
