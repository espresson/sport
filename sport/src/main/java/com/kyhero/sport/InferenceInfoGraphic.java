...
import com.kyhero.sport.Fragment.SportFragment;
...
public class InferenceInfoGraphic extends GraphicOverlay.Graphic {
    ...
    public synchronized void draw(Canvas canvas) {
        ...
        if (SportFragment.debugmode) {
            canvas.drawText(
                    "图片尺寸: " + overlay.getImageHeight() + "x" + overlay.getImageWidth(),
                    x,
                    y,
                    textPaint);
            if (!showLatencyInfo) {
            ...
            if (framesPerSecond != null) {
                canvas.drawText(
                        "FPS: " + framesPerSecond + ", 延时: " + frameLatency + " ms",
                        x,
                        y + TEXT_SIZE,
                        textPaint);
            } else {
                canvas.drawText("延时: " + frameLatency + " ms", x, y + TEXT_SIZE, textPaint);
            }
        }
